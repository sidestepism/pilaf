...
for category in ['チャーハン', '能年玲奈']:
	files = glob.glob('./surf-prediction/%s/*.surf' % category)
	for file in files:
...
file = open('histograms-prediction.dat', 'w')
...
file = open('histograms-prediction_label.dat', 'w')
for histogram in histograms:
	file.write(str(histogram[0]) + "\n")
file.close()

file = open('histograms-prediction_data.dat', 'w')
for histogram in histograms:
	file.write(' '.join(map(lambda x: str(x), histogram[1])) + "\n")
file.close()
